...
bool ChessGame::isCheck() {
    Position kingPos;
    // Znajdü pozycjÍ krÛla obecnego gracza
    ...
    // Sprawdü, czy jakakolwiek figura przeciwnika moøe zaatakowaÊ pozycjÍ krÛla
...
bool ChessGame::isCheckmate() {
    ...
    // Sprawdü, czy obecny gracz ma jakiekolwiek legalne ruchy, ktÛre wyprowadzπ go z szacha
    for (int row = 0; row < 8; ++row) {
        for (int col = 0; col < 8; ++col) {
...
bool ChessGame::isStalemate() {
    ...
    // Sprawdü, czy obecny gracz ma jakiekolwiek legalne ruchy
    for (int row = 0; row < 8; ++row) {
        for (int col = 0; col < 8; ++col) {
            if (board[row][col].color == currentPlayer) {
                std::vector<Position> possibleMoves = getPossibleMoves({ row, col });
